<!-- filepath: c:\xampp\htdocs\UrbanStyle\resources\views\sale_payments\index.blade.php -->
@extends('layouts.app')
...
<div class="container mx-auto p-6 bg-white shadow-md rounded-lg">
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold">Sale Payments</h1>
        <a href="{{ route('sale_payments.create') }}" class="bg-gray-800 text-white px-4 py-2 rounded">New Payment</a>
    </div>

    <!-- Search Bar -->
    <form method="GET" action="{{ route('sale_payments.index') }}" class="mb-4">
        ...
        <tbody>
            @forelse($salePayments as $payment)
                <tr class="hover:bg-gray-100">
                    ...
                    <td class="p-2 border">{{ $payment->saleInvoice->invoice_number ?? 'N/A' }}</td>
                    <td class="p-2 border">{{ $payment->saleInvoice->customer->name ?? 'N/A' }}</td>
            ...
            @empty
                <tr>
                    <td colspan="8" class="p-2 border text-center">No payments found</td>
                </tr>
            @endforelse
        </tbody>
    </table>
