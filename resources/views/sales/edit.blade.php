...
<div class="container mx-auto p-6 bg-white shadow-md rounded-lg">
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('sales.update', $saleInvoice->id) }}" method="POST" x-data="{
                ...
                totalAmount: 0,
                calculateTotals() {
                    this.subtotal = this.items.reduce((sum, item) => sum + (item.quantity * item.price), 0).toFixed(2);
                    this.taxAmount = (this.subtotal * (this.taxRate / 100)).toFixed(2);
                    this.totalAmount = (parseFloat(this.subtotal) + parseFloat(this.taxAmount)).toFixed(2);
                }
            }"
            x-init="$watch('items', () => calculateTotals())"
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input type="date" name="invoice_date" id="invoice_date" class="form-control mt-1 block w-full" value="{{ $saleInvoice->invoice_date }}" required>
                    </div>
                </div>
